...
14. Escribe HTML a `/tmp/{candidate}_{company}.html`
15. Lee `pdf_output_dir` de `config/profile.yml` (default: `output/`). Ejecuta: `node generate-pdf.mjs /tmp/{candidate}_{company}.html "{pdf_output_dir}/{candidate}_{company}_{YYYY-MM-DD}.pdf" --format={letter|a4}`
