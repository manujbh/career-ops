...
15. Lee `pdf_output_dir` de `config/profile.yml` (default: `output/`). Ejecuta: `node generate-pdf.mjs /tmp/cv-{candidate}-{company}.html "{pdf_output_dir}/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf" --format={letter|a4}`
