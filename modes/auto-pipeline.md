...
1. **Exa web_fetch (preferred):** Use `mcp__plugin_everything-claude-code_exa__web_fetch_exa` to extract the JD. Works with SPAs and static pages.
2. **WebFetch (fallback):** For static pages if Exa fails (ZipRecruiter, WeLoveProduct, company career pages).
...
Run exactly as the `oferta` mode (read `modes/oferta.md` for all blocks A-F + Block G Posting Legitimacy).
...
1. **Extract form questions**: Navigate to the form and take a snapshot if possible. If questions cannot be extracted, use the generic questions below.
